...
if mode == "Manual Input":
    ...
    if st.button("🔮 Predict"):
        ...
        # Use decision_function + sigmoid for binary pseudo-probability
        score = model.decision_function(input_scaled)[0]
        prob_good = 1 / (1 + np.exp(-score)) * 100
...
# -------------------------
    ...
    if file is not None:
        ...
        prob_good = 1 / (1 + np.exp(-scores))
        prob_bad = 1 - prob_good

        df["Prediction"] = preds
        df["Prob_NotApproved"] = prob_bad * 100
        df["Prob_Approved"] = prob_good * 100
